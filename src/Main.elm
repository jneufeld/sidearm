...
init : Model
init = Model [] ["Fold", "Raise", "Fold", "Call"]

-- UPDATE
type Msg = PreviousAction | NextAction

update : Msg -> Model -> Model
update msg model = case msg of
    PreviousAction -> processPrevAction model
    NextAction -> processNextAction model

processPrevAction : Model -> Model
processPrevAction model = case model.prevActions of
    [] -> model
    _ -> { model |
...
processNextAction model = case model.nextActions of
    [] -> model
    current :: rest -> { model |
...
actionText actions = case actions of
    [] -> "None"
    action :: _ -> action